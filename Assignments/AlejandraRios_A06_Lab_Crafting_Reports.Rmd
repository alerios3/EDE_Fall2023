...
```{r project.setup, message=FALSE, warning=FALSE, errors = FALSE}
#remove messages, errors, or outputs
...
#Display the Mean Particulates (2.5mm) table
cat("Mean Particulates (2.5mm)\n")
kable(summary_PM25, format = "markdown", col.names = c("County", "$\\mu g/m^3$"))
#used chatgbt to get cat(\n) to get table title
...
#Display the Mean Ozone table
cat("Mean Ozone\n")
kable(summary_Ozone, format = "markdown", col.names = c("County", "$\\mu g/m^3$"))

#Use the `kable` function to produce the 2 tables in markdown format (above)
```
...
# Create a boxplot for Ozone levels by month in 2018
boxplot(Ozone ~ month(Date), data = ozone_2018, xlab = "Month", ylab = "Ozone Level (ppm)")
```
...
# Create a boxplot for Ozone levels by month in 2019
boxplot(Ozone ~ month(Date), data = ozone_2019, xlab = "Month", ylab = "Ozone Level (ppm)")
#filter ozone by month using ~ found on https://www.rdocumentation.org/packages/graphics/versions/3.6.2/topics/boxplot
```
